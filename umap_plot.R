# @param umap.data Data frame or matrix: if dataframe/matrix should have samples as columns and rows as features
...
umap_plot <- function(umap.data, labels=FALSE, printres=FALSE, seed=FALSE, axistextsize = 18,
                      ...
                      colvec = c("skyblue", "gold", "violet", "darkorchid", "slateblue", "forestgreen", 
                                 "violetred", "orange", "midnightblue", "grey31", "black"), text = FALSE){
  ...
  ## plotting function (depending on whether labels or text are needed)
  scores <- data.frame(umap.data$layout)
